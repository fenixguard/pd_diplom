...
from jobs.mixins import SuperSelectableMixin, ViewSetViewSerializersMixin, ViewSetViewDescriptionsMixin
from jobs.models import Category, Shop, ProductInfo, Product, ProductParameter, Order, OrderItem
from jobs.partner_info_loader import load_partner_info
from jobs.permissions import IsShop, IsBuyer
from jobs.response import ResponseOK, ResponseCreated, ResponseBadRequest, ResponseForbidden, ResponseConflict
from jobs.utils import to_positive_int, is_dict
...
shared_user_properties = {
    ...
    'action_descriptions': {
        ...
        'retrieve': _('Информация о пользователе')
    },
...
class BaseUserViewSet(SuperSelectableMixin,
    ...
    def register(self, request, *args, **kwargs):
        ...
        """
        serializer = self.get_serializer_class()(data=request.data,
                                                 context={'request': request, 'user_type': self.user_type})
        serializer.is_valid(raise_exception=True)
...
class ContactViewSet(ViewSetViewSerializersMixin, ViewSetViewDescriptionsMixin, viewsets.ModelViewSet):
    ...
    action_descriptions = {
        'list': _('Список контактов текущего пользователя'),
        'retrieve': _('Контакт текущего пользователя'),
    }
...
class OrderViewSet(ViewSetViewSerializersMixin, ViewSetViewDescriptionsMixin, viewsets.ReadOnlyModelViewSet):
    ...
    action_descriptions = {
        'list': _('Список заказов текущего пользователя'),
        'retrieve': _('Заказ текущего пользователя'),
        'create': _('Оформить заказ')
    }
...
class BasketViewSet(ViewSetViewSerializersMixin, ViewSetViewDescriptionsMixin, viewsets.GenericViewSet):
    ...
    action_descriptions = {
        'list': _('Список заказов в корзине текущего пользователя'),
        'retrieve': _('Заказ в корзине текущего пользователя'),
    }
        ...
        try:
            with transaction.atomic():
                ...
                for order_item in items_list:
                    if not is_dict(order_item) or type(order_item.get('id')) != int or type(
