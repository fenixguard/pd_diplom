...
from jobs.models import Category, Shop, ProductInfo, Product, ProductParameter, OrderItem, Order
from jobs.serializers import DefaultSerializer, DefaultModelSerializer, ModelPresenter
from jobs.utils import is_dict
from jobs.validators import NotBlankTogetherValidator, EqualTogetherValidator
from rest_auth.models import User, Contact, ADDRESS_ITEMS_LIMIT


class PartnerUpdateSerializer(DefaultSerializer):
    url = serializers.URLField(required=False, help_text=_('Путь к загружаемому файлу'), label=_('URL path'))
    file = serializers.FileField(required=False, help_text=_('Файл, передаваемый через http'), label=_('File field'))
    ...
    def validate(self, data):
        ...
        if not phone and not all((city, street, house,)):
            raise serializers.ValidationError(dict(
                Errors=_('Не указаны все необходимые аргументы. Нужно указать телефон или город/улицу/дом')))
...
class UserLoginSerializer(DefaultSerializer):
    email = serializers.EmailField(required=True, allow_blank=False, label=_('Email'), help_text=_('Input email'))
    password = serializers.CharField(required=True, allow_blank=False, style={'input_type': 'password'},
                                     label=_('Password'), help_text=_('Input password'))
    token = serializers.CharField(read_only=True, label=_('Auth token'), help_text=_('Auth token'))
    recaptcha = ReCaptchaField(write_only=True)


class CaptchaInfoSerializer(DefaultSerializer):
    public_key = serializers.CharField(read_only=True, label=_('reCaptcha public key'),
                                       help_text=_('reCaptcha public key'))
...
class RegisterUserSerializer(DefaultModelSerializer):
    ...
    password2 = serializers.CharField(write_only=True, required=True, allow_blank=False,
                                      style={'input_type': 'password'},
                                      label=_('Repeat Password'), help_text=_('Input password again'),
                                      validators=(validate_password,))
    recaptcha = ReCaptchaField(write_only=True)
    ...
    class Meta:
        model = User
        fields = (
            'url', 'id', 'first_name', 'last_name', 'email', 'company', 'position', 'password', 'password2',
            'recaptcha',
            'contacts', 'Errors', 'Status',)
    ...
    def validate(self, data):
        ...
        if contacts and (not isinstance(contacts, list) or len(contacts) > ADDRESS_ITEMS_LIMIT):
            raise serializers.ValidationError(
                dict(contacts=_('Число контактов должно быть не более {}').format(ADDRESS_ITEMS_LIMIT)))
        return super(RegisterUserSerializer, self).validate(data)


class RetrieveUserDetailsSerializer(DefaultModelSerializer):
    ContactSerializer = ModelPresenter(Contact, (
        'url', 'person', 'phone', 'city', 'street', 'house', 'structure', 'building', 'apartment'))
    ...
    class Meta:
        model = User
        fields = (
            'url', 'id', 'first_name', 'last_name', 'email', 'company', 'position', 'contacts', 'Errors', 'Status',)
...
class UpdateUserDetailsSerializer(DefaultModelSerializer):
    ...
    class Meta:
        model = User
        fields = (
            'url', 'id', 'first_name', 'last_name', 'email', 'company', 'position', 'password', 'contacts', 'Errors',
            'Status',)
        read_only_fields = ('url', 'id',)
            ...
            for contact in contact_data:
                try:
                    ...
                    instance.contacts.add(contact)
                        ...
                        DBError, ValidationError, ObjectDoesNotExist, PermissionDenied, FieldError,
...
class ConfirmUserSerializer(DefaultSerializer):
    email = serializers.EmailField(required=True, allow_blank=False, label=_('Email'), help_text=_('Input email'))
    token = serializers.CharField(required=True, allow_blank=False, label=_('Auth token'), help_text=_('Auth token'))
...
class ProductParameterSerializer(DefaultModelSerializer):
    parameter = serializers.StringRelatedField(label=_('Параметр товара'), help_text=_('Параметр товара'))
...
class ProductInfoSerializer(DefaultModelSerializer):
    ...
    product = ProductSerializer(read_only=True, label=_('Товар'), help_text=_('Данные по товару'))
    product_parameters = ProductParameterSerializer(read_only=True, many=True, label=_('Параметры'),
                                                    help_text=_('Параметры товара'))
    shop = ShopSerializer(read_only=True, label=_('Магазин'), help_text=_('Данные по магазину'))

    class Meta:
        model = ProductInfo
        fields = (
            'url', 'id', 'product', 'shop', 'quantity', 'price', 'price_rrc', 'product_parameters', 'Errors', 'Status',)
        read_only_fields = ('url', 'id',)
...
class ShowBasketSerializer(DefaultModelSerializer):
    ...
    ProductInfoSerializer = ModelPresenter(ProductInfo, ('id', 'url', 'product', 'shop', 'price', 'price_rrc'),
    ...
    ordered_items = OrderedItemsSerializer(read_only=True, many=True, label=_('Заказанные товары'),
                                           help_text=_('Заказанные товары'))

    total_sum = serializers.DecimalField(read_only=True, max_digits=20, decimal_places=2, min_value=0, label=_('Total'),
                                         help_text=_('Общая сумма'))
...
class OrderSerializer(DefaultModelSerializer):
    ...
    total_sum = serializers.DecimalField(read_only=True, max_digits=20, decimal_places=2, min_value=0, label=_('Total'),
                                         help_text=_('Общая сумма'))
    contact = ContactSerializer(read_only=True, label=_('Контакт'),
                                help_text=_('Контактные данные, указанные заказчиком'))
    ...
    class KeyField(serializers.PrimaryKeyRelatedField):
        def get_queryset(self):
            if Order.objects.filter(user_id=self.context['request'].user.id, state='basket').exists():
                return Order.objects.get(user_id=self.context['request'].user.id,
                                         state='basket').ordered_items.prefetch_related(
